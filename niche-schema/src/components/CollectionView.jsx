                  ...
                  >
                    ...
                    <TableBody>
                      <TableRow>
                        ...
                        {Object.keys(data[props.id][fields[0]]).map(
                          (key, index) => {
                            let schema = columnsSchema[fields[0]];
                            let details = data[props.id][fields[0]][key];

                            if (
                              schema[key].hasOwnProperty("required") &&
                              ...
                              if (schema[key]["type"].includes("|")) {
                                let currentType =
                                  schema[key]["type"].split(" | ");
                                if (!currentType.includes(details)) {
                                  errors++;
                                }
                              } else if (
                                schema[key]["type"] !== typeof details
                              ) {
                                errors++;
                              }
                            ...
                            if (
                              ...
                              details.hasOwnProperty("seconds")
                            ) {
